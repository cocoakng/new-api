...
// FailoverGroupsArray is a []string that serializes to/from JSON for DB storage.
type FailoverGroupsArray []string

func (f *FailoverGroupsArray) Scan(value interface{}) error {
	if value == nil {
		*f = nil
		return nil
	}
	...
	case string:
		data = []byte(v)
	default:
		return errors.New("failover_groups: invalid type")
	}
	if len(data) == 0 {
		*f = nil
		return nil
	}
	return common.Unmarshal(data, f)
}

func (f FailoverGroupsArray) Value() (driver.Value, error) {
	if len(f) == 0 {
		return "", nil
	}
	return common.Marshal(f)
}
...
func (token *Token) Update() (err error) {
	...
	err = DB.Model(token).Select("name", "status", "expired_time", "remain_quota", "unlimited_quota",
		"model_limits_enabled", "model_limits", "allow_ips", "group", "cross_group_retry", "failover_groups").Updates(token).Error
	return err
}
