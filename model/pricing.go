...
type Pricing struct {
	...
	PerCallMatrix          *billing_setting.PerCallMatrixData `json:"per_call_matrix,omitempty"`
}
...
func updatePricing() {
	...
	for model, groups := range modelGroupsMap {
		...
		if billingMode := billing_setting.GetBillingMode(model); billingMode == billing_setting.BillingModePerCall {
			if matrix, ok := billing_setting.GetPerCallMatrix(model); ok && len(matrix.Resolutions) > 0 && len(matrix.Durations) > 0 {
				pricing.BillingMode = billingMode
				pricing.PerCallMatrix = &matrix
				pricing.QuotaType = 3 // per_call_matrix
			}
		}
